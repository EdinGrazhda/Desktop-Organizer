...
import { normalizeSlotIcon } from "../utils/icons";
import { normalizePathKey } from "../utils/path";

const SLOTS_KEY = "desknest_slots";
const SLOTS_BACKUP_KEY = "desknest_slots_backup";
  ...
  return {
    id:
      typeof input.id === "string" && input.id.trim().length > 0
        ...
        : createId("slot"),
    name,
    icon: normalizeSlotIcon(typeof input.icon === "string" ? input.icon : ""),
    color:
...
function decodeStoredSlots(raw: string | null): Slot[] | null {
  if (!raw) return null;
...
function readLocalSlots(): Slot[] | null {
  try {
    const primary = decodeStoredSlots(localStorage.getItem(SLOTS_KEY));
    const loadedFromBackup = !primary;
    const normalized =
      primary ?? decodeStoredSlots(localStorage.getItem(SLOTS_BACKUP_KEY));
...
function writeLocalSlots(slots: Slot[]): void {
  try {
    ...
    localStorage.setItem(SLOTS_KEY, serialized);
    localStorage.setItem(SLOTS_BACKUP_KEY, serialized);
  } catch (err) {
...
export function updateSlot(
  ...
  const icon =
    updates.icon === undefined
      ? slots[idx].icon
      : normalizeSlotIcon(updates.icon);
  slots[idx] = {
    ...slots[idx],
    ...updates,
    icon,
