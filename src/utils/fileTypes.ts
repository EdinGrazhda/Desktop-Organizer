...
import { isWindowsShellVirtualPath } from "./path";
...
function findSlotByKeywords(slots: Slot[], keywords: string[]): Slot | null {
  return (
    slots.find((s) => keywords.some((k) => s.name.toLowerCase().includes(k))) ??
    null
  );
}
...
function looksLikeGameTarget(filename: string, fullPath?: string): boolean {
  ...
  if (
    nonGameHints.some(
      (hint) => lowerName.includes(hint) || lowerPath.includes(hint),
    )
  ) {
    return false;
  }

  return gameNameHints.some(
    (hint) => lowerName.includes(hint) || lowerPath.includes(hint),
  );
}
...
export function detectItemType(path: string, isDir = false): SlotItem["type"] {
  ...
  if (isWindowsShellVirtualPath(path)) return "shortcut";
 ...
 */
export function findBestMatchingSlot(
  slots: Slot[],
  filename: string,
  fullPath?: string,
): Slot | null {
