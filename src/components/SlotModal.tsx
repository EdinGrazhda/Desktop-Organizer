...
export function SlotModal({
  ...
  useEffect(() => {
    if (editSlot) {
      ...
      setIcon(normalizeSlotIcon(editSlot.icon));
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!name.trim()) return;
    onSave(name.trim(), normalizeSlotIcon(icon), color, description.trim());
    onClose();
  }
    ...
    >
      <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl w-full max-w-md mx-4 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
            ...
            <div className="grid grid-cols-5 gap-2">
              {SLOT_ICONS.map((ic) => (
                <button
                  ...
                  type="button"
                  onClick={() => setIcon(ic)}
                  title={getSlotIconLabel(ic)}
                  className={`flex items-center justify-center p-2 rounded-xl border transition-all ${icon === ic ? "bg-white/15 border-white/35 ring-1 ring-white/30" : "bg-white/5 border-white/10 hover:bg-white/10"}`}
                >
                  <SlotIcon
                    icon={ic}
                    className={`w-4 h-4 ${icon === ic ? "text-white" : "text-gray-300"}`}
                    strokeWidth={2}
                  />
                </button>
              ))}
          ...
          {/* Preview */}
          <div
            className={`flex items-center gap-3 p-3 rounded-xl border ${getSlotColors(color).border} ${getSlotColors(color).bg}`}
          >
            <span
              className={`relative w-10 h-10 rounded-2xl border ${getSlotColors(color).border} ${getSlotColors(color).iconShell} ${getSlotColors(color).iconGlow} flex items-center justify-center overflow-hidden`}
            >
              <span
                className={`absolute inset-[2px] rounded-[11px] border border-white/15 ${getSlotColors(color).iconInner}`}
              />
              <SlotIcon
                icon={icon}
                className={`relative w-4.5 h-4.5 ${getSlotColors(color).text} drop-shadow-[0_0_12px_rgba(255,255,255,0.2)]`}
                strokeWidth={2}
              />
            </span>
            <div>
              <p className={`text-sm font-medium ${getSlotColors(color).text}`}>
                {name || "Slot Name"}
              </p>
              {description && (
