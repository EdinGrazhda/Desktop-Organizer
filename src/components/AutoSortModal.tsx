...
export function AutoSortModal({
  ...
  async function handleSortAll() {
    ...
    const moveCount = rows.filter(
      (r) =>
        r.included &&
        r.targetSlotId &&
        (moveFilesPhysically ||
          isDesktopOriginPath(r.candidate.path, desktopPath)),
    ).length;
    ...
    for (const row of rows) {
      ...
      const shouldMove =
        Boolean(targetSlot) &&
        (moveFilesPhysically ||
          isDesktopOriginPath(candidate.path, desktopPath));
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-white truncate">
                      {row.candidate.isDir
                        ? "📁"
                        : getFileEmoji(
                            getFileExtension(row.candidate.name),
                          )}{" "}
                      {row.candidate.name}
                    </p>
...
function getFileEmoji(ext: string): string {
  ...
  if (["pdf", "doc", "docx", "txt", "xlsx"].includes(ext)) return "📄";
