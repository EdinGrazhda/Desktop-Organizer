  ...
  const handleDrop = useCallback(
    (paths: string[], slotId: string | null) => {
      ...
      if (slotId) {
        ...
        const moveCount = pendingPaths.filter((path) => {
          const name = path.split(/[\\/]/).pop() ?? path;
          const isDir = !name.includes(".");
          const itemType = detectItemType(path, isDir);
        ...
        void (async () => {
          ...
          for (const originalPath of pendingPaths) {
            ...
            const originalIsDir = !originalName.includes(".");
            const originalType = detectItemType(originalPath, originalIsDir);
            const shouldMove =
              Boolean(targetSlot) &&
