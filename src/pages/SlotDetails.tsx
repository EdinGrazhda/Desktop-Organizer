...
export function SlotDetails({
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    const shouldMove = shouldMoveItemToSlot({
      path: finalPath,
      itemType: addType,
      moveFilesPhysically: settings.moveFilesPhysically,
      customDesktopPath: settings.desktopPath,
    });
    if (shouldMove) {
      const targetName =
      ...
      <div className={`px-6 pt-6 pb-4 border-b border-white/5`}>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span
              className={`relative w-11 h-11 rounded-2xl border ${colors.border} ${colors.iconShell} ${colors.iconGlow} flex items-center justify-center overflow-hidden`}
            >
              <span
              ...
              <SlotIcon
                icon={slot.icon}
                className={`relative w-5.5 h-5.5 ${colors.text} drop-shadow-[0_0_12px_rgba(255,255,255,0.2)]`}
                strokeWidth={2}
              />
            </span>
            <div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4">
        {slot.items.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-3 text-center">
            <span
              className={`relative w-16 h-16 rounded-3xl border ${colors.border} ${colors.iconShell} ${colors.iconGlow} flex items-center justify-center overflow-hidden`}
            >
              <span
                className={`absolute inset-[3px] rounded-[20px] border border-white/15 ${colors.iconInner}`}
              />
              <SlotIcon
                icon={slot.icon}
                className={`relative w-8 h-8 ${colors.text} opacity-90 drop-shadow-[0_0_12px_rgba(255,255,255,0.2)]`}
                strokeWidth={2}
              />
            </span>
